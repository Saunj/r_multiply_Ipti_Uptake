...
my.fields <- c("record_id","hf_district1" ,
               ...
               'ipti_m6_fixed','ipti_m6_outreach','ipti_m12_fixed','ipti_m12_outreach',
               'ipti_m15_fixed','ipti_m15_outreach')
...
total_Ac.data <-iptiuptake[filter, ]

filter <- iptiuptake$report_period_since >= first_date_of_period

period_Ac.data <-iptiuptake[filter,c('record_id','hf','hf_name','report_period_since',
                                         'ipti1','ipti2','ipti3','iptim6','iptim12','iptim15')]

total_Acc <- nrow(total_Ac.data)

period_Acc <- nrow(period_Ac.data)

write.csv(total_Ac.data , file = "Total_Accountability_Iptiuptake.csv", row.names = F)

write.csv(period_Ac.data, file = "Period_Accountability_Iptiuptake.csv", row.names = F)
